...
module.exports = async (req, res) => {
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      const inv = invDoc.exists ? invDoc.data() : { afkFish: [] };
      const afkFish = inv.afkFish || [];

      if (afkFish.length === 0) { earlyError = { error: 'No AFK fish to sell.' }; return; }

      const earned = afkFish.reduce((s, f) => s + (f.value || 0), 0);
      const newCoins = (user.coins || 0) + earned;

      t.update(userRef, { coins: newCoins });
      t.set(invRef, { afkFish: [] }, { merge: true });

      result = { success: true, count: afkFish.length, totalValue: earned, newBalance: newCoins };
    });
  } catch (e) {
    console.error('afk-sell tx failed', e);
